...
class ManaDashboardFieldInfo(models.Model):
    ...
    show_time = fields.Boolean(
        string='Show Time', compute='_compute_show_time')
    ...
    get_last_period = fields.Boolean(
        string='Get Last Period',
        default=False)
    show_get_last_period = fields.Boolean(
        string='Show Get Last Period',
        compute='_compute_show_compare_with_last_period')
    ...
    def _compute_show_compare_with_last_period(self):
        ...
        for record in self:
            if record.field.ttype in ['date', 'datetime']:
                ...
                record.show_get_last_period = True
            else:
                record.show_compare_with_last_period = False
                record.show_get_last_period = False
    ...
    @api.depends('datetime_range', 'datetime_range', 'get_last_period')
    def _compute_previous(self):
        """
        compute previous start time and end time
        """
        for record in self:
            if record.datetime_range and record.datetime_range.range_type != 'custom':
                if record.get_last_period:
                    record.range_start_time = record.datetime_range.previous_start_time
                    record.range_end_time = record.datetime_range.previous_end_time
                    record.previous_start_time = record.datetime_range.previous_start_time
                    record.previous_end_time = record.datetime_range.previous_end_time
                    record.start_time = record.datetime_range.previous_start_time
                    record.end_time = record.datetime_range.previous_end_time
                else:
                    record.range_start_time = record.datetime_range.start_time
                    record.range_end_time = record.datetime_range.end_time
                    record.previous_start_time = record.datetime_range.previous_start_time
                    record.previous_end_time = record.datetime_range.previous_end_time
                    record.start_time = record.datetime_range.start_time
                    record.end_time = record.datetime_range.end_time
            else:
                record.range_start_time = False
                record.range_end_time = False
                record.previous_start_time = False
                record.previous_end_time = False

    @api.onchange('datetime_range', 'datetime_range', 'get_last_period')
    def _onchange_previous(self):
        """
        onchange previous start time and end time
        """
        self._compute_previous()