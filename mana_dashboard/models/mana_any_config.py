...
class ManaDashboardAnyConfig(models.Model):
    ...
    def import_any_config(self, any_config_info):
        ...
        # bind config
        config.any_config_id = result.id
        return result
        
    # clone config
    def _clone_config(self, options={}):
        """
        clone config
        :param options:
        :return:
        ...
        # delete config name to avoid the same name
        del config_info['config_info']['config_name']
        new_config = self.import_any_config(config_info)

        # set the dashboard id
        new_config.dashboard_id = self.dashboard_id.id
        new_config.ref_config.dashboard_id = self.dashboard_id.id

        return new_config
    
    def clone_config(self, custom_config_infos, options={}):
        """
        clone and get config
        """
        new_config = self._clone_config(options)
        
        # clone config
        custom_config_infos = []
        for custom_config_info in custom_config_infos:
            res_model = custom_config_info.get('res_model')
            res_id = custom_config_info.get('res_id')
            if res_model and res_id:
                custom_config = self.env[res_model].browse(res_id).copy({
                    'config_id': new_config.id
                })
                custom_config_infos.append({
                    'res_model': res_model,
                    'res_id': custom_config.id,
                    'name': custom_config_info.get('name'),
                    'key': custom_config_info.get('key')
                })
        return {
            'config_id': new_config.id,
            'ref_config_id': new_config.ref_config.id,
            'custom_config_infos': custom_config_infos
        }
