...
class ManaDashboardParameter(models.Model):
    ...
    default_start = fields.Char(string='Default Start')
    default_end = fields.Char(string='Default End')
    ...
    def get_parameters(self):
        ...
        for parameter in self:
            if parameter.type in ['date_time_range', 'date_range']:
                paramaters_dict[parameter.name] = {
                    'name': parameter.name,
                    'type': parameter.type,
                    'value': {
                        'start': parameter.default_start,
                        'end': parameter.default_end
                    }
                }
            else:
                paramaters_dict[parameter.name] = {
