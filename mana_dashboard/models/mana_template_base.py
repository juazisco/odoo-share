...
class ManaDashboardTemplateBase(models.Model):
    ...
    multi_data_source = fields.Boolean(
        string='Multi Data Source', 
        related='template_id.multi_data_source')
    
    data_source_mode = fields.Selection(
        string='Data Source Mode',
        selection=[
            ('single', 'Single'), 
            ('multi', 'Multi'),
            ('mixed', 'Mixed')],
    ...
    has_scripts = fields.Boolean(
        string='Has Scripts', 
        related='template_id.has_scripts')

    default_scripts = fields.Text(
        string='Default Scripts')

    disable_children = fields.Boolean(
        string='Disable Children', 
        related='template_id.disable_children')
    
    disable_first_child = fields.Boolean(
        string='Disable First Child', 
        related='template_id.disable_first_child')
    ...
    need_measure = fields.Boolean(
        string='Need Measure', 
        related='template_id.need_measure')
    
    need_category = fields.Boolean(
        string='Need Category',
        related='template_id.need_category')
    
    need_column_aggregation = fields.Boolean(
        string='Need Column Aggregation', 
        related='template_id.need_column_aggregation')
