...
class ManaDataSourceMixin(models.Model):
    ...
    def get_data_source_info(self):
        ...
        data_source_info = {
            ...
            'is_valid': False,
            'fields_as_category': self.fields_as_category,
        ...
        if data_source_type_name == 'model':
            ...
            if self.code:
                data_source_info['is_valid'] = True
        elif data_source_type_name == 'none':
            data_source_info['is_valid'] = True
    ...
    def get_data_from_model(self, option, get_field_infos=True):
        ...
        for group_by_info in group_by_infos:
            title = group_by_info['field_description'] or group_by_info[
                'field_name'] or group_by_info['full_name']
        ...
        if get_field_infos:
            for model_field in model_fields:
                ...
                if name_cache.get(name):
                    continue
                # check if it is in group by infos
                field_infos.append({
                    'field': name,
                    'field_name': name,
            ...
            for order_by_info in order_by_infos:
                ...
                field_infos.append({
                    'field': name,
                    'field_name': name,
                    'field_type': order_by_info.get('field_type'),
                    'string': title,
                    'title': title,
                })
        ...
        extra_fields = self.env['ir.model.fields'].sudo().browse(extra_field_ids)

        extra_fields_names = extra_fields.mapped('name')
        for order_by_info in order_by_infos:
            if order_by_info.get('name'):
                extra_fields_names.append(order_by_info['name'])
            elif order_by_info.get('field_name'):
                extra_fields_names.append(order_by_info['field_name'])
        ...
        group_by_names = [
            group_by_info['full_name'] for group_by_info in group_by_infos if group_by_info['full_name']]
        group_by_names = list(set(group_by_names))
        order_by_names = [
            order_by_info['full_name'] for order_by_info in order_by_infos if order_by_info['full_name']]
        ...
        if len(group_by_infos) > 0:
            datas = self.env[model.model].sudo().with_context(context or {}).read_group(
                domain=domain,
                fields=field_names,
    ...
    def xw_import(self, module, eval_context={}):
        """
        import module
        """
        if module not in eval_context:
            tmp = __import__(module)
            names = dir(tmp)
            atrributes = {}
            for name in names:
                atrributes[name] = getattr(tmp, name)
                target = getattr(tmp, name)
                if inspect.ismodule(target):
                    atrributes[name] = getattr(target, '__all__', [])
                else:
                    atrributes[name] = name
            eval_context[module] = wrap_module(tmp, atrributes)

    def xw_import_from(self, module, atrributes, eval_context={}):
        """
        import module from names
        """
        tmp = __import__(module)
        for name in atrributes:
            atrributes[name] = getattr(tmp, name)
            target = getattr(tmp, name)
            if inspect.ismodule(target):
                atrributes[name] = getattr(target, '__all__', [])
                ...
                atrributes[name] = name
        eval_context[module] = wrap_module(module, atrributes)
    ...
    def get_data_from_code(self, code, extra_context={}, get_field_infos=True):
        ...
        if 'params' not in eval_context:
            eval_context['params'] = Box(eval_context.get('params', {}))
        eval_context['xw_import'] = self.xw_import
        eval_context['xw_import_from'] = self.xw_import_from
        eval_context['result'] = None

        # get all the import xxx
        import_modules = re.findall(r'import\s+([\w\.]+)', code)
        for module in import_modules:
            self.xw_import(module, eval_context)
        # replace import xxxx with ''
        code = re.sub(r'import\s+([\w\.]+)', '', code)

        # get all the from xxx import yyy
        import_from_modules = re.findall(r'from\s+([\w\.]+)\s+import\s+([\w\.]+)', code)
        for module, names in import_from_modules:
            self.xw_import_from(module, names, eval_context)
        # replace from xxxx import yyy with ''
        code = re.sub(r'from\s+([\w\.]+)\s+import\s+([\w\.]+)', '', code)

        safe_eval(code, eval_context, mode="exec", nocopy=True) 
        result = eval_context.get('result')

        # covert style
        if self.result_type != self.get_custom_result_type():
            if len(result) > 0:
                first_record = result[0]
                if isinstance(first_record, (list, tuple)):
                    names = result[0]
                    datas = result[1:]
                    result = [dict(zip(names, data)) for data in datas]
    ...
    def _get_field_infos(self, results, extra_context, result_type=None):
        """
        get field infos
        """
        field_infos = []
    ...
    def get_datas(self, options={}):
        ...
        if data_source_type.name == 'model':
                ...
                for search_info in search_infos:
                    ...
                    if key in model_field_cache or key in model_search_key_cache:
                        ...
                        for model_field in self.model_fields:
                            ...
                            if model_field.affect_by_global and model_field.field_type in ['datetime', 'date'] and type in ['datetime_range', 'date_range', 'date']:
                                if type == 'datetime_range':
                                    start = search_info.get('start')
                                    end = search_info.get('end')
                                    domain = expression.AND(
            ...
            if not options.get('fetch_sql_data', True):
                return result
            extra_context = self.get_extra_context(options)
            return self.get_data_from_sql(self.sql, extra_context)
        elif data_source_type.name == 'method':
            if not options.get('fetch_model_method_data', True):
                return result
            model_id = self.model.id
            extra_context = self.get_extra_context(options)
            return self.get_data_from_model_method(model_id, self.method, extra_context)
            ...
            return self.get_data_from_json(self.json_data, extra_context)
        elif data_source_type.name == 'code':
            if not options.get('fetch_code_data', True):
                return result
            extra_context = self.get_extra_context(options)
            return self.get_data_from_code(self.code, extra_context)
    ...
    def _get_eval_context(self):
        ...
        context = {
            ...
            'json5': wrap_module(json5, getattr(json5, '__all__', [])),
            'pendulum': wrap_module(pendulum, getattr(pendulum, '__all__', [])),
        }
        return context
