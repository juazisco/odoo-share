...
class Home(http.Controller):
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if request.httprequest.method == 'POST':
            ...
            try:
                ...
                if e.args == odoo.exceptions.AccessDenied().args:
                    values['error'] = _("Error en usuario/contraseña")
                else:
                    values['error'] = e.args[0]
        else:
            if 'error' in request.params and request.params.get('error') == 'access':
                values['error'] = _('Solo usuario pueden acceder a esta base de datos, contacte con su adminsitrador.')
        ...
        second_logo_image = param_obj.get_param('login_background.second_logo_image')
        values['display_info'] = param_obj.get_param('login_background.display_info')
        ...
        image_url = ''
        if second_logo_image:
            base_url = param_obj.get_param('web.base.url')
            image_url = base_url + '/web/image?' + 'model=login.image&id=' + second_logo_image + '&field=image'
            values['second_logo_src'] = image_url or ''

        #ALERT MESSAGE
        values['login_alert_type'] = param_obj.get_param('login.alert.type')
        values['login_alert_message'] = param_obj.get_param('login.alert.message','')
        
        if style == 'default' or style is False:
            response = request.render('web.login', values)
        elif style == 'left':
            response = request.render('login_background.left_login_template', values)
        elif style == 'right':
            response = request.render('login_background.right_login_template', values)
        else:
            response = request.render('login_background.middle_login_template', values)
